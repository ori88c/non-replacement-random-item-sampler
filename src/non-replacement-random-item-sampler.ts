 ...
 */
export class NonReplacementRandomSampler<T> {
    private _currentCycle: number = 1;
    private _availablePrefixLength: number;
    ...
    constructor(items: T[]) {
        if (items.length <= 1) {
            throw new Error(
                `At least 2 items are required for random sampling; received ${items.length}`
            );
        }
     ...
     */
    public get remainingItemsInCurrentCycle(): number {
     ...
     */
    public sampleGroupFromCycle(groupSize: number): T[] {
        if (groupSize > this._availablePrefixLength) {
            throw new Error(
                `Cannot sample ${groupSize} unique items from cycle ${this._currentCycle}: ` +
                `only ${this._availablePrefixLength} items remain`
            );
        }

        const uniqueSamples: T[] = [];
