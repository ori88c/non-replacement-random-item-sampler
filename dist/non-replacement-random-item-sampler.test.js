...
describe('NonReplacementRandomSampler tests', () => {
    describe('Happy path tests', () => {
        test('each item is sampled exactly once within each cycle', async () => {
            ...
            const items = new Array(itemsCount)
                .fill(0)
                .map((_, index) => index);
            const sampler = new non_replacement_random_item_sampler_1.NonReplacementRandomSampler(items);
            for (let cycleNumber = 1; cycleNumber <= cyclesCount; ++cycleNumber) {
                const alreadySampledItems = new Set();
    ...
    describe('Negative path tests', () => {
        test('should throw when number of items is less than 2', () => {
            expect(() => new non_replacement_random_item_sampler_1.NonReplacementRandomSampler([])).toThrow();
            expect(() => new non_replacement_random_item_sampler_1.NonReplacementRandomSampler([354])).toThrow();
        });
        test('sampleGroupFromCycle: throws an error if group size exceeds the number of remaining items in the current cycle', () => {
            const itemsCount = 34;
            const items = new Array(itemsCount)
                .fill(0)
                .map((_, index) => index);
...
//# sourceMappingURL=non-replacement-random-item-sampler.test.js.map