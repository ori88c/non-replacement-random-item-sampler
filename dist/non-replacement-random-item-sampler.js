...
exports.NonReplacementRandomSampler = void 0;
/**
 ...
 */
class NonReplacementRandomSampler {
    /**
     * Constructor
     *
    ...
    constructor(items) {
        ...
        if (items.length <= 1) {
            throw new Error(`At least 2 items are required for random sampling; received ${items.length}`);
        }
        this._items = items;
        this._availablePrefixLength = items.length;
    }
    /**
     * size
     *
     ...
     */
    get remainingItemsInCurrentCycle() {
        return this._availablePrefixLength;
    }
    /**
     * sample
     *
     ...
     */
    sample() {
        const randomAvailableIndex = Math.floor(Math.random() * this._availablePrefixLength);
        const sampledItem = this._items[randomAvailableIndex];
        // Decrease the length of the available items prefix (from which we choose), and
        // expand the unavailable items suffix.
        // This is done by swapping the reference of the chosen item with the new start
        // of the unavailable items suffix.
        if (--this._availablePrefixLength !== randomAvailableIndex) {
            this._items[randomAvailableIndex] = this._items[this._availablePrefixLength];
            this._items[this._availablePrefixLength] = sampledItem; // Unavailable until the next cycle.
        }
        // If all items have been sampled in the current cycle (i.e., the cycle is exhausted),
        // trigger the next cycle.
        if (this._availablePrefixLength === 0) {
            this.startNewCycle();
        }
        return sampledItem;
    }
     ...
     */
    sampleGroupFromCycle(groupSize) {
        if (groupSize > this._availablePrefixLength) {
            throw new Error(`Cannot sample ${groupSize} unique items from cycle ${this._currentCycle}: ` +
                `only ${this._availablePrefixLength} items remain`);
        }
        const uniqueSamples = [];
        for (let i = 0; i < groupSize; ++i) {
            uniqueSamples.push(this.sample());
        }
     ...
     */
    startNewCycle() {
        this._availablePrefixLength = this._items.length;
        ++this._currentCycle;
    }
}
exports.NonReplacementRandomSampler = NonReplacementRandomSampler;
//# sourceMappingURL=non-replacement-random-item-sampler.js.map